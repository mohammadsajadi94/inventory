...
namespace inventory
{
    public partial class maxforosh : Form
    {
        ...
        private async void maxforosh_Load(object sender, EventArgs e)
        {
            cn = new SqlConnection("Data Source = .;Initial Catalog = teamwork;Integrated Security = True");
            cn.Open();
            SqlCommand cm = new SqlCommand("SELECT TOP 100 ModifiedDate AS N'تاریخ ثبت',FactorID AS N'شماره فاکتور',PRUDUCTID AS N'شماره کالا',CATEGORY N'نوع کالا',PNAME N'نام کالا',PRICE N'قیمت',TEDAD N'تعداد',DISCONT AS N'درصد تخفیف',TOTALPRICE AS N'قیمت کل',NAMEMOSH AS N'نام مشتری',EMPLOYEID N'شماره پرسنلی ثبت کننده',NAME N'نام ثبت کننده',FAMILY N'نام خانوادگی ثبت کننده',SEMAT N'سمت ثبت کننده'   FROM Factor_forosh ORDER BY TEDAD DESC ", cn);
